...
function refresh(){
	window.location.href='showReportMajorCount.action?year='+$('#year').val()+'&type='+$('#type').val();
}
...
function showReportName(){
	var html = '申通地铁集团'+$('#year').val()+'年轨道交通运营设施设备大修和更新改造项目资金计划完成情况表(汇总表）<span style="float:right;">单位(万元)</span>';
	$('#h').html(html);
}
							...
							<td style="white-space:nowrap" width="15%;">
								<select id="reportType" name="reportType" class="input_slarge" class="input_large" onchange="changeReport(this.value);">
										<s:if test="type == 4 || type == null">
											...
											<option value="22" selected="selected">报表2.2年度资金计划完成情况表(汇总表)</option>
...
<div align="center" id = "h">
	申通地铁集团年轨道交通运营设施设备大修项目资金计划完成情况表(汇总表）<span style="float:right;">单位(万元)</span>
</div>
<!--Tabs_2 End-->
