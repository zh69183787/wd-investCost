...
function countAll(){
	var feeMoney=0.0,contractPrice=0.0,finalPrice=0.0;
	$('#dataArea').find("tr").each(function(index){
		if(index!=$('#dataArea').find("tr").length-1){
			if($.trim($(this).find('td:eq(12)').html())!=null && $.trim($(this).find('td:eq(12)').html())!=''){
				feeMoney += parseFloat($.trim($(this).find('td:eq(12)').html()));
			}
			if($.trim($(this).find('td:eq(15)').html())!=null && $.trim($(this).find('td:eq(15)').html())!=''){
				contractPrice += parseFloat($.trim($(this).find('td:eq(15)').html()));
			}
			if($.trim($(this).find('td:eq(16)').html())!=null && $.trim($(this).find('td:eq(16)').html())!=''){
				finalPrice +=parseFloat($.trim($(this).find('td:eq(16)').html()));
			}
		}
	});
	$('#countAll').find("td:eq(11)").html(feeMoney.toFixed(4));
	$('#countAll').find("td:eq(14)").html(contractPrice.toFixed(4));
	$('#countAll').find("td:eq(15)").html(finalPrice.toFixed(4));
}

function showReportName(){
	var html = '上海申通地铁集团有限公司'+$('#year').val()+'年度轨道交通运营设施设备大修和更新改造项目资金计划完成情况表(项目详表)<span style="float:right;">单位(万元)</span>';
	$('#h').html(html);
}
							...
							<td style="white-space:nowrap">
								<select id="reportType" name="reportType" class="input_slarge" class="input_large" onchange="changeReport(this.value);">
									<s:if test="type == 4 || type == null">
											...
											<option value="21" selected="selected">报表2.1年度资金计划完成情况表(项目详表)</option>
...
<div align="center" id = "h">
	  	上海申通地铁集团有限公司20   年度轨道交通运营设施设备大修和更新改造项目资金计划完成情况表(项目详表)<span style="float:right;">单位(万元)</span>
</div>
<!--Tabs_2 End-->
	...
	<table width="100%"  class="table_1" style="overflow: scroll;">
		<thead style="background: white;">
			<tr class="nwarp" >
                ...
                <td class="t_c" rowspan="2" style="vertical-align:middle" >项目实施进度</td>
                <td class="t_c" rowspan="1" colspan="2"  >本年度用款计划</td>
                <td class="t_c" rowspan="2" style="vertical-align:middle" width="15%;">项目下合同名称</td>
                ...
                <td class="t_c" rowspan="1" colspan="2"  >委托方式</td>
                <td class="t_c" rowspan="2" style="vertical-align:middle" >合同价</td>
                <td class="t_c" rowspan="2" style="vertical-align:middle" >决算价<br>（若有）</td>
                <td class="t_c" rowspan="1" colspan="2"  >至上年底开累支付</td>
                <td class="t_c" rowspan="1" colspan="2"  >本年度累计已支付</td>
                <td class="t_c" rowspan="1" colspan="2"  >至本年底开累支付</td>
            </tr>
            <tr class="nwarp" >
            	<td class="t_c" >总数</td>
                <td class="t_c" >其中政府<br>补贴数</td>
                <td class="t_c" >指定维修</td>
                <td class="t_c" >招标</td>
                <td class="t_c" >总数</td>
                <td class="t_c" >其中政府<br>补贴数</td>
                <td class="t_c" >总数</td>
		...
		<tbody id="dataArea">
			<s:iterator id="vo" value="#request.list" status="st">
				<tr>
	                ...
	                <td class="t_r"><s:property value="#vo.projectProgress"/></td>
	                <td class="t_r"><s:property value="#vo.thisYearPlanCount"/></td>
	                <td class="t_r"><s:property value="#vo.thisYearPlanGov"/></td>
	                <td><s:property value="#vo.contractName"/></td>
	                <td><s:property value="#vo.feeDepartment"/></td>
	                <td class="t_r"><s:property value="#vo.feeMoney"/></td>
	                <td><s:property value="#vo.appointMaintain"/></td>
	                <td><s:property value="#vo.bidInvaiting"/></td>
	                <td class="t_r"><s:property value="#vo.contractPrice"/></td>
	                <td class="t_r"><s:property value="#vo.finalPrice"/></td>
	                <td class="t_r"><s:property value="#vo.lastYearCount"/></td>
	                <td class="t_r"><s:property value="#vo.lastYearGov"/></td>
	                <td class="t_r"><s:property value="#vo.thisYearPayCount"/></td>
	                <td class="t_r"><s:property value="#vo.thisYearPayGov"/></td>
	                <td class="t_r"><s:property value="#vo.endYearPayCount"/></td>
	                <td class="t_r"><s:property value="#vo.endYearPayGov"/></td>
	            </tr>
			</s:iterator>
				<tr id="countAll">
