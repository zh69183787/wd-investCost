...
function showTitle(){
	...
	$('#dataArea').find('tr').each(function(index){
		var feeDepartment = $(this).find('td:eq(5)').html().split(' ');
		alert(feeDepartment);
...
function countAll(){
	...
	$('#dataArea').find("tr").each(function(index){
		$(this).find('td:eq(5)').html($(this).find('td:eq(5)').html().replaceAll('\n','<br>'));	//费用承担单位换行		
		...
		if(index!=$('#dataArea').find("tr").length-1){
			...
			td13 = getNumberCount(td13,$(this).find('td:eq(14)').html());
			td14 = getNumberCount(td14,$(this).find('td:eq(15)').html());
		}
	});
	...
	$('#countAll').find("td:eq(9)").html(contractPrice!=null?finalPrice.toFixed(6):contractPrice);
	
	$('#countAll').find("td:eq(10)").html(td10!=null?td10.toFixed(6):td10);
	$('#countAll').find("td:eq(11)").html(td11!=null?td11.toFixed(6):td11);
	$('#countAll').find("td:eq(13)").html(td13!=null?td13.toFixed(6):td13);
	$('#countAll').find("td:eq(14)").html(td14!=null?td13.toFixed(6):td13);
}

function refresh(){
	window.location.href='showReportProjectProgress.action?year='+$('#year').val()+'&season='+$('#season').val()+'&type='+$('#type').val();
}

function showReportName(){
	var html = '上海申通地铁集团有限公司'+$('#year').val()+'年度第'+$('#season').val()+'季度轨道交通运营设施设备大修和更新改造项目资金计划完成情况季度表(实施进展情况表)<span style="float:right;">单位(万元)</span>';
	$('#h').html(html);
}
	...
	<div class="ctrl clearfix">
		...
		<div class="posi fl">
			<ul>
				...
				<li class="fin">季度资金计划完成情况表(实施进展情况表)</li>
			</ul>
		</div>
				...
				<s:form action="showReport4" name="dwProject" namespace="/dwProject" method="post" id="form">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr id="search1">
							...
							<td style="white-space:nowrap">
								<select id="reportType" name="reportType" class="input_slarge" class="input_large" onchange="changeReport(this.value);">
									<s:if test="type == 4 || type == null">
											...
											<option value="32" selected="selected">报表3.2季度资金计划完成情况表(实施进展情况表)</option>
									</s:if>
								</select>
							</td>
							...
							<td class="t_r" style="white-space:nowrap;" width="5%;">季度</td>
							<td style="white-space:nowrap" width="100%;">
								<select name="year" id="season" onchange="refresh();">
									<s:if test="#request.season==1">
										<option value="1" selected="selected">一季度</option>
										<option value="2">二季度</option>
										<option value="3">三季度</option>
										<option value="4">四季度</option>
									</s:if>
									<s:elseif test="#request.season==2">
										<option value="1">一季度</option>
										<option value="2" selected="selected">二季度</option>
										<option value="3">三季度</option>
										<option value="4">四季度</option>
									</s:elseif>
									<s:elseif test="#request.season==3">
										<option value="1">一季度</option>
										<option value="2">二季度</option>
										<option value="3" selected="selected">三季度</option>
										<option value="4">四季度</option>
									</s:elseif>
									<s:elseif test="#request.season==4">
										<option value="1">一季度</option>
										<option value="2">二季度</option>
										<option value="3">三季度</option>
										<option value="4" selected="selected">四季度</option>
									</s:elseif>
								</select>
							</td>
	...
	<table width="100%"  class="table_1" style="overflow: scroll;">
		<thead style="background: white;">
			<tr class="nwarp" >
                ...
                <td class="t_c" rowspan="2">项目实施进度</td>
